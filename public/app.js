(() => {
  ...
  function loadSettings() {
    try {
      ...
      if (raw) {
        ...
        if (s.selectedColor) selectedColor = s.selectedColor;
      }
    } catch {}
  }
  ...
  function selectColor(color, activeBtn) {
    ...
    myColorsList.querySelectorAll('.my-color').forEach(b => b.classList.remove('active'));
    myColorsList.querySelectorAll('.my-color').forEach(b => {
      if (b.style.background === color || b.title === color) b.classList.add('active');
    });
    saveSettings();
  }
  ...
  saveCustomColorBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      saveCustomColorBtn.textContent = 'Save';
      saveCustomColorBtn.classList.remove('saved');
    }, 1200);
  });

  // --- Color Mixer ---
  mixerToggleBtn.addEventListener('click', () => {
    colorMixer.classList.toggle('hidden');
    mixerToggleBtn.classList.toggle('active');
  });

  mixerCloseBtn.addEventListener('click', () => {
    colorMixer.classList.add('hidden');
  ...
  function updateMixResult() {
    ...
    mixPreviewA.style.background = a;
    mixPreviewB.style.background = b;
  ...
  useMixBtn.addEventListener('click', () => {
    const mixed = updateMixResult();
    selectColor(mixed, null);
  });

  saveMixBtn.addEventListener('click', () => {
    const mixed = updateMixResult();
    if (!savedColors.includes(mixed)) {
      savedColors.push(mixed);
      saveSavedColors();
      renderSavedColors();
    }
  });
  ...
  useHslBtn.addEventListener('click', () => {
    const hex = updateHslResult();
    selectColor(hex, null);
  });

  saveHslBtn.addEventListener('click', () => {
    const hex = updateHslResult();
  ...
  function renderNote(note) {
    ...
    const time = formatTime(note.created);
    ...
    el.innerHTML = `
      <div class="note-header" data-role="drag-handle">
        ...
        <div class="note-actions">
          <button data-action="delete" title="Delete note">&times;</button>
        </div>
      </div>
      <div class="note-body">
        <textarea placeholder="Type your note...">${escapeHtml(note.text)}</textarea>
      </div>
      <div class="note-footer">
        <div class="note-color-pick" data-action="pick-color" title="Pick color" style="background:${note.color}">
    ...
    el.addEventListener('mousedown', (e) => {
      ...
      if (target.dataset.action === 'recolor') {
        note.color = target.dataset.color;
        el.style.backgroundColor = note.color;
        const pickDot = el.querySelector('.note-color-pick');
        if (pickDot) pickDot.style.background = note.color;
        saveNotes();
        return;
      }

      if (target.dataset.action === 'pick-color') {
        const colorInput = target.querySelector('input[type="color"]') || target;
        const handler = (ev) => {
          note.color = ev.target.value;
          el.style.backgroundColor = note.color;
          target.style.background = ev.target.value;
          saveNotes();
        };
        colorInput.addEventListener('input', handler, { once: true });
        colorInput.click();
        return;
      }

      if (target.dataset.role === 'resize-handle') {
        e.preventDefault();
        startResize(e, note, el);
        return;
      }

      if (target.dataset.role === 'drag-handle') {
        bringToFront(el);
        startDrag(e, note, el);
      }
    });
  ...
  // --- Drag ---
  function startDrag(e, note, el) {
    dragState = { note, el, offsetX: e.clientX - note.x, offsetY: e.clientY - note.y };
  ...
  function onDrag(e) {
    if (!dragState) return;
    const { note, el, offsetX, offsetY } = dragState;
  ...
  function startResize(e, note, el) {
    resizeState = { note, el, startX: e.clientX, startWidth: note.width };
  }

  function onResize(e) {
    if (!resizeState) return;
    const { note, el, startX, startWidth } = resizeState;
    note.width = Math.max(180, Math.min(500, startWidth + (e.clientX - startX)));
    el.style.width = note.width + 'px';
    autoResize(el.querySelector('textarea'));
  }

  function stopResize() {
    if (resizeState) { resizeState = null; saveNotes(); }
  }
  ...
  if (isElectron) {
    ...
    window.electronAPI.onClearAll(() => clearBtn.click());
    window.electronAPI.onShowAbout(() => {
      alert('Quick Note Sticky Board v1.0.0\n\nCross-platform desktop sticky notes app built with Electron.');
    });
  }
})();
